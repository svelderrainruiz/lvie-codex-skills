...
Describe 'Release workflow contract' {
    ...
    It 'publishes release assets from CI artifacts plus installer' {
        ...
        $script:releaseContent | Should -Match 'Download Windows container x86 shadow PPL bundle artifact\s*[\s\S]*?continue-on-error:\s*true'
        $script:releaseContent | Should -Match 'Download Linux container x86 shadow PPL bundle artifact\s*[\s\S]*?continue-on-error:\s*true'
        $script:releaseContent | Should -Match 'Download self-hosted windows x64 shadow PPL bundle artifact\s*[\s\S]*?continue-on-error:\s*true'
        ...
        $script:releaseContent | Should -Match 'Optional shadow lane assets:'
        $script:releaseContent | Should -Match 'missing shadow lane artifacts do not block release publishing'
